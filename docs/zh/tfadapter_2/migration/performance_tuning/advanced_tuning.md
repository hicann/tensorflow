...
- 通过对迭代轨迹数据进行分析，若发现“FP to BP Time”耗时较长，可进一步根据op_statistic_\*.csv与op_summary_\*.csv文件，分析耗时较长的算子。
    ...
    若存在AI CPU算子，需进一步分析其执行时间段是否能够被AI Core算子的执行时间所覆盖。如若不能，则应考虑将该AI CPU算子通过AI Core算子的方式重新实现，详细的实现方法可参见《[Ascend C算子开发指南](https://hiascend.com/document/redirect/CannCommercialOpdevAscendC)》。
