...
- 若[op_debug_level](../..//apiref/session_config/deprecated_options.md#op_debug_level)配置非0值，会忽略op_compiler_cache_mode的配置，不启用算子编译缓存功能，算子全部重新编译。
- 若[op_debug_config](../../apiref/session_config/debugging.md#op_debug_config)配置非空，且配置文件中未配置op_debug_list字段，会忽略op_compiler_cache_mode的配置，不启用算子编译缓存功能，算子全部重新编译。
- 若[op_debug_config](../../apiref/session_config/debugging.md#op_debug_config)配置非空，且配置文件中配置了op_debug_list字段，当op_compiler_cache_mode配置为enable或force时，列表中的算子会重新编译，列表外的算子会启用算子编译缓存，不再重新编译。
...
关于环境变量ASCEND_CACHE_PATH的详细说明可参见《[环境变量参考](https://hiascend.com/document/redirect/CannCommunityEnvRef)》中的“安装配置相关”章节。
