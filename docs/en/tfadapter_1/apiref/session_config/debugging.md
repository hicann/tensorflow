...
Enable global memory check. The value is the path of the .cfg configuration file. Multiple options in the configuration file are separated by commas (,).
...
Directory of the debug files generated during operator compilation, including the .o, .json, and .cce files.
