...
- If analysis of the iteration trace data shows that the  **FP to BP Time**  is long, analyze the time-consuming operators based on the  **op_statistic_\*.csv**  and  **op_summary_\*.csv**  files.
    ...
    If AI CPU operators exist, check whether their execution time segments can be covered by the execution time of AI Core operators. If no, implement the AI CPU operators in AI Core operator mode. For details, see  [Ascend C Operator Development](https://hiascend.com/en/document/redirect/CannCommercialOpdevAscendC).
