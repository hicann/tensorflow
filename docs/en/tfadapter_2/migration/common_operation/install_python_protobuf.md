...
4. Install Protobuf as the  **root**  user.
    ...
    3. Configure the installation path. The default installation path is  **/usr/local**.<a id="4.c"></a>
    ...
    5. Refresh the shared libraries.
        ...
        After Protobuf is installed, the  **google/protobuf**  folder is generated in the  **include**  directory in  [4.c](#4.c). This folder stores the header files related to Protobuf. Generate the  **protoc**  executable file in the  **bin**  directory in  [4.c](#4.c). The  **protoc**  executable file is used to compile the  **\*.proto**  file and generate the C++ header file and implementation file of Protobuf.
    ...
    6. Check whether the installation is complete.
        ...
        **/protobuf**  is the installation path configured in  [4.c](#4.c). If the installation path is not configured, run the  **protoc --version**  command to check whether the installation is successful.
...
5. Install the runtime library of Protobuf Python.
    ...
    4. If you have specified the installation path in  [4.c](#4.c), add the following environment variable settings to the run script:

        ```bash
        export LD_LIBRARY_PATH=/protobuf/lib:${LD_LIBRARY_PATH}
        ```

        **/protobuf**  is the installation path configured in  [4.c](#4.c).
