...
TEST_F(NpuAttrTest, GetAllAttrOptions_oo_level) {
  AttrValueMap attr_map;
  ...
  oo_level.set_s("O3");
  attr_map["_oo_level"] = oo_level;

  AttrSlice attrs(&attr_map);
  const auto &all_options = NpuAttrs::GetAllAttrOptions(attrs);
  EXPECT_NE(all_options.find("ge.oo.level"), all_options.cend());
}

TEST_F(NpuAttrTest, SetNpuOptimizerAttr_optimization_switch) {
  AttrValueMap attr_map;
