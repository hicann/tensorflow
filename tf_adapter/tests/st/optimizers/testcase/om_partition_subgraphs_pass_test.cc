...
TEST_F(OmOptimizationPassTest, UnaryOpsTest) {
  ...
  InitGraph(org_graph_def_path);
  std::string target_graph = DoRunOmOptimizationPassTest(true);
  bool ret = target_graph.find("_Floor") != target_graph.npos && target_graph.find("_Abs") != target_graph.npos;
  EXPECT_EQ(ret, true);
}
...
TEST_F(OmOptimizationPassTest, MixCompileSaveV2Test) {
  std::string org_graph_def_path = "tf_adapter/tests/ut/optimizers/pbtxt/om_test_mix_compile_savev2.pbtxt";
  InitGraph(org_graph_def_path);
  auto ret = DoRunOmOptimizationPassTest();
  EXPECT_NE(ret.find("IdentityN"), std::string::npos);
}
...
TEST_F(OmOptimizationPassTest, NpuOpsIdentifierTest04) {
  ...
  setenv("ASCEND_OPP_PATH", opp_path.c_str(), 1);
  ge::SetCustomPathStub(custom_opp_path);
  std::string path_builtin = opp_path + "built-in";
  std::string path_vendors = opp_path + "vendors";
  std::string path_config = path_vendors + "/config.ini";
  system(("mkdir -p " + path_builtin).c_str());
...
TEST_F(OmOptimizationPassTest, NpuOpsIdentifierTest06) {
  ...
  setenv("ASCEND_OPP_PATH", opp_path.c_str(), 1);
  ge::SetCustomPathStub((custom_opp_path_01 + ":" + custom_opp_path_02));
  std::string path_builtin = opp_path + "built-in";
  std::string path_vendors = opp_path + "vendors";
  std::string path_config = path_vendors + "/config.ini";
  system(("mkdir -p " + path_builtin).c_str());
