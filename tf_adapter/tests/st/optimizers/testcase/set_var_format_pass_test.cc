...
namespace {
class SetVarFormatPassTest : public testing::Test {
 public:
  SetVarFormatPassTest() : graph_(absl::make_unique<Graph>(OpRegistry::Global())) {}
  ...
  string DoRunSetVarFormatPassTest() {
    string before = CanonicalGraphString(graph_.get());
    LOG(INFO) << "Before set var format pass: " << before;
    ...
    SetVarFormatPass().Run(options);
    unsetenv("PRINT_MODEL");

    string result = CanonicalGraphString(options.graph->get());
    LOG(INFO) << "After set var format pass: " << result;
    return result;
  }
...
TEST_F(SetVarFormatPassTest, DumpTest) {
  ...
  EXPECT_EQ(DoRunSetVarFormatPassTest(), target_graph);
}
} // end namespace
} // end tensorflow
