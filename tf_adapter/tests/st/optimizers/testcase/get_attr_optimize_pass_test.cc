...
TEST_F(GetAttrOptimizationPassTest, SetAttrTest) {
  ...
  const int64_t set_model_num = 8LL;
  AttrValue graph_max_parallel_model_num = AttrValue();
  graph_run_mode.set_i(set_model_num);
  (*custom_config->mutable_parameter_map())["graph_max_parallel_model_num"] = graph_max_parallel_model_num;
  ...
  AttrValue input_fusion_size = AttrValue();
  input_fusion_size.set_i(330223);
  (*custom_config->mutable_parameter_map())["input_fusion_size"] = input_fusion_size;
...
TEST_F(GetAttrOptimizationPassTest, InputFusionSizeLessZero) {
  string org_graph_def_path = "tf_adapter/tests/ut/optimizers/pbtxt/get_attr_job_chief_test.pbtxt";
  InitGraph(org_graph_def_path);
  std::string target_graph = "VariableV2->Identity;Const->Add;Identity->Add:1;Add->_Retval";
  SessionOptions session_options;
  ...
  (*custom_config->mutable_parameter_map())["job"] = job;
  AttrValue input_fusion_size = AttrValue();
  input_fusion_size.set_i(-23);
  (*custom_config->mutable_parameter_map())["input_fusion_size"] = input_fusion_size;
  EXPECT_EQ(DoRunGetAttrOptimizationPassTest(session_options), target_graph);
}

TEST_F(GetAttrOptimizationPassTest, InputFusionSizeMore32M) {
  string org_graph_def_path = "tf_adapter/tests/ut/optimizers/pbtxt/get_attr_job_chief_test.pbtxt";
  InitGraph(org_graph_def_path);
  std::string target_graph = "VariableV2->Identity;Const->Add;Identity->Add:1;Add->_Retval";
  SessionOptions session_options;
  ...
  (*custom_config->mutable_parameter_map())["job"] = job;
  AttrValue input_fusion_size = AttrValue();
  input_fusion_size.set_i(329000000000);
  (*custom_config->mutable_parameter_map())["input_fusion_size"] = input_fusion_size;
  EXPECT_EQ(DoRunGetAttrOptimizationPassTest(session_options), target_graph);
}
