...
TEST(BasicLSTMCellCStateGradOpTest, TestBasicLSTMCellCStateGrad) {
  ...
  ASSERT_FALSE(basic_lstm_cell_cstate.IsExpensive());
  delete device;
  delete node_def;
  delete op_def;
  delete context;
...
TEST(BasicLSTMCellCStateGradOpTest, TestBasicLSTMCellCStateGradShapeInference) {
  ...
  shape_inference::InferenceContext c(0, &def, op_def,
                                      {TShape({1, 27957}), TShape({1, 9319}), TShape({1}), TShape({1, 57000}),
                                       TShape({1, 2}), TShape({3, 7200}), TShape({3, 7200}), TShape({3, 7200})},
                                      {}, {}, {});
