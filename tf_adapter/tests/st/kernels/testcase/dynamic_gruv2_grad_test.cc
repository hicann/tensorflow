...
TEST(DynamicGruV2GradTest, TestDynamicGruV2Grad) {
  std::vector<DataType> in_types_vec = {DT_FLOAT, DT_FLOAT, DT_FLOAT, DT_FLOAT, DT_FLOAT, DT_FLOAT, DT_FLOAT,
  ...
  ASSERT_FALSE(dynamic_gruv2_grad.IsExpensive());
  delete device;
  delete node_def;
  delete op_def;
  delete context;
...
TEST(DynamicGruV2GradOpTest, TestDynamicGruV2GradShapeInference) {
  ...
  shape_inference::InferenceContext c(
      0, &def, op_def,
      {TShape({1, 16, 16}), TShape({16, 48}), TShape({16, 48}), TShape({16, 16}), TShape({16, 16}), TShape({16, 16}),
