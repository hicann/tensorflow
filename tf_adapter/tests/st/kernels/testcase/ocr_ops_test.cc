...
TEST(OCROpsTest, TestOCRDetectionPreHandleShapeInference) {
  ...
  shape_inference::InferenceContext c(0, &def, op_def, {TShape({3, 3, 3})}, {}, {}, {});
  ASSERT_TRUE(reg->shape_inference_fn(&c).ok());
}
...
TEST(OCROpsTest, TestOCRIdentifyPreHandleShapeInference) {
  ...
  TF_CHECK_OK(NodeDefBuilder("dummy", &op_def)
                  ...
                  .Input(FakeInputStub(DT_INT32))
                  .Finalize(&def));
  shape_inference::InferenceContext c(0, &def, op_def, {TShape({10}), TShape({3}), TShape({3, 3})}, {}, {}, {});
  ASSERT_TRUE(reg->shape_inference_fn(&c).ok());
}
...
TEST(OCROpsTest, TestOCRIdentifyPreHandleShapeInference1) {
  ...
  TF_CHECK_OK(NodeDefBuilder("dummy", &op_def)
                  ...
                  .Input(FakeInputStub(DT_INT32))
                  .Finalize(&def));
  shape_inference::InferenceContext c(0, &def, op_def, {TShape({3}), TShape({3}), TShape({3, 3})}, {}, {}, {});
  ASSERT_TRUE(reg->shape_inference_fn(&c).ok());
}
