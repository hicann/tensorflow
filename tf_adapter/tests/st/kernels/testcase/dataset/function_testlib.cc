...
NodeDef NDef(StringPiece names, StringPiece opereator, gtl::ArraySlice<string> input,
             gtl::ArraySlice<std::pair<string, FDH::AttrValueWrapper>> attr, const string &device_id) {
  NodeDef n;
...
FunctionDef MakeRangeDataset() {
  return FDH::Define("MakeRangeDataset", {"start: int64", "stop: int64", "step: int64"}, {"y:variant"},
...
FunctionDef MakeTakeDataset() {
  return FDH::Define("TakeDataset", {"input_dataset: variant", "count: int64"}, {"y:variant"},
                     {"output_types: list(type) >= 1", "output_shapes: list(shape) >= 1"},
                     {{{"y"},
                       "TakeDataset",
                       {"input_dataset", "count"},
...
FunctionDef MakeTensorSliceDataset() {
  return FDH::Define("MakeTensorSliceDataset", {"x: Toutput_types"}, {"y: variant"},
                     {"Toutput_types: list(type) >= 1", "output_shapes: list(shape) >= 1"},
                     {{{"y"},
                       "TensorSliceDataset",
                       {"x"},
                       {{"Toutput_types", "$Toutput_types"}, {"output_shapes", "$output_shapes"}}}});
}
