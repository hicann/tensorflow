...
extern void RewriteInputShapeOption(std::string &str);
...
class NpuGetNextOutputInfo {
 public:
  ...
  ~NpuGetNextOutputInfo() {
    LOG(INFO) << "[GEOP] Release NpuGetNextOutputInfo.";
  }
...
Status GeOpRunGraphAsync(std::string example_path, gtl::InlinedVector<TensorValue, 4> inputs, NodeDef &geop_node_def,
  ...
  for (int i = 0; i < graph_def.node_size(); i++) {
    ...
    if (node_def->name() == node_name) {
      ...
      while (CallbackExecutor::GetInstance().GetRunNum() > 0) {
        usleep(100);
      }
    }
  }
...
Status GeOpRunGraphAsyncMultiStep(std::string example_path, std::vector<gtl::InlinedVector<TensorValue, 4>> inputs,
                                  NodeDef &geop_node_def, std::string node_name) {
  Env *env = Env::Default();
  GraphDef graph_def;
  std::string graph_def_path = example_path;
  ReadTextProto(env, graph_def_path, &graph_def);
...
TEST_F(GeOpTest, GeOpInitTest) {
  NpuClose();
  PluginFinalize();
  CallbackExecutor::GetInstance().Init();
  NodeDef node_def;
  std::string graph_def_path = "tf_adapter/tests/ut/kernels/pbtxt/geop.pbtxt";
  gtl::InlinedVector<TensorValue, 4> inputs;
...
TEST_F(GeOpTest, GeOpFuncTestWithProfiling) {
  NpuClose();
  NodeDef node_def;
  std::string graph_def_path = "tf_adapter/tests/ut/kernels/pbtxt/geop.pbtxt";
  gtl::InlinedVector<TensorValue, 4> inputs;
  EXPECT_TRUE(ProfilerStart("L1", "MemoryUB", "./").empty());
  EXPECT_TRUE(GeOpRunGraphAsync(graph_def_path, inputs, node_def, "GeOp1_0").ok());
  EXPECT_TRUE(ProfilerStop().empty());
}

TEST_F(GeOpTest, GeOpFuncTestWithLifeCycleControl) {
  NpuClose();
  NodeDef node_def;
  std::string graph_def_path = "tf_adapter/tests/ut/kernels/pbtxt/geop.pbtxt";
  gtl::InlinedVector<TensorValue, 4> inputs;
  EnableControl();
  EXPECT_TRUE(ScopedGraphManager::Instance().IsControlEnabled() == true);
  EXPECT_TRUE(GeOpRunGraphAsync(graph_def_path, inputs, node_def, "GeOp1_0").ok());
  Clear();
  EXPECT_TRUE(ScopedGraphManager::Instance().IsControlEnabled() == false);
}

TEST_F(GeOpTest, GeOpFuncTestEmptyGraphWithLifeCycleControl) {
  NpuClose();
  NodeDef node_def;
  std::string graph_def_path = "tf_adapter/tests/ut/kernels/pbtxt/geop.pbtxt";
  gtl::InlinedVector<TensorValue, 4> inputs;
...
TEST_F(GeOpTest, GeOpFuncTestWithProfilingDefaultAicMetircs) {
  NpuClose();
  NodeDef node_def;
  std::string graph_def_path = "tf_adapter/tests/ut/kernels/pbtxt/geop.pbtxt";
  gtl::InlinedVector<TensorValue, 4> inputs;
  EXPECT_TRUE(ProfilerStart("L1", "", "./").empty());
  EXPECT_TRUE(GeOpRunGraphAsync(graph_def_path, inputs, node_def, "GeOp1_0").ok());
  EXPECT_TRUE(ProfilerStop().empty());
}

TEST_F(GeOpTest, GeOpFuncTestWithProfilingErrorLevelAicMetrics) {
  NpuClose();
  NodeDef node_def;
  std::string graph_def_path = "tf_adapter/tests/ut/kernels/pbtxt/geop.pbtxt";
  gtl::InlinedVector<TensorValue, 4> inputs;
  EXPECT_EQ(ProfilerStart("L0", "PipeUtilization", "./"), "Please use L1 or L2 if you want to collect aic metrics!");
}

TEST_F(GeOpTest, GeOpFuncTestWithProfilingErrorLevel) {
  NpuClose();
  NodeDef node_def;
  std::string graph_def_path = "tf_adapter/tests/ut/kernels/pbtxt/geop.pbtxt";
  gtl::InlinedVector<TensorValue, 4> inputs;
  EXPECT_EQ(ProfilerStart("L4", "PipeUtilization", "./"),
            "Profiler options: level cannot set to: L4, should set level to [L0, L1, L2].");
}

TEST_F(GeOpTest, GeOpFuncTestWithProfilingErrorAicMetrics) {
  NpuClose();
  NodeDef node_def;
  std::string graph_def_path = "tf_adapter/tests/ut/kernels/pbtxt/geop.pbtxt";
  gtl::InlinedVector<TensorValue, 4> inputs;
  EXPECT_EQ(ProfilerStart("L1", "PipeUtiliza", "./"),
            "Profiler options: aic_metrics cannot set to: PipeUtiliza, should set aic_metrics to "
            "[ArithmeticUtilization, L2Cache, Memory, MemoryL0, MemoryUB, PipeUtilization, ResourceConflictRatio].");
}

TEST_F(GeOpTest, GeOpFuncTest_NpuCompile) {
  NpuClose();
  NodeDef node_def;
  std::string graph_def_path = "tf_adapter/tests/ut/kernels/pbtxt/geop_npu_compile.pbtxt";
  gtl::InlinedVector<TensorValue, 4> inputs;
  EXPECT_TRUE(GeOpRunGraphAsync(graph_def_path, inputs, node_def, "GeOp1_0").ok());
}
...
TEST_F(GeOpTest, GeOpJitCompileFalseTest) {
  ...
  auto getnext_output1_info = std::unique_ptr<NpuGetNextOutputInfo>(
      new NpuGetNextOutputInfo(ge::kPlacementDevice, ge_output1_dims, 8, nullptr));
  ...
  auto getnext_output2_info = std::unique_ptr<NpuGetNextOutputInfo>(
      new NpuGetNextOutputInfo(ge::kPlacementDevice, ge_output2_dims, 8, nullptr));
  ...
  Tensor d(DT_INT32, TensorShape({4}));
  gtl::InlinedVector<TensorValue, 4> inputs2{TensorValue(&a), TensorValue(&b), TensorValue(&d)};
  EXPECT_TRUE(GeOpRunGraphAsyncMultiStep(graph_def_path, {inputs, inputs2}, node_def, "GeOp11_1").ok());
}
...
TEST_F(GeOpTest, GeOpGetNextStringTest) {
  ...
  std::vector<int64_t> ge_output1_dims{2, 2};
  auto getnext_output1_info = std::unique_ptr<NpuGetNextOutputInfo>(
      new NpuGetNextOutputInfo(ge::kPlacementDevice, ge_output1_dims, 128, nullptr));
  Allocator *allocator1 = NpuHostGetNextAllocator::Create(std::move(getnext_output1_info));
  Tensor a(allocator1, DT_INT64, TensorShape({2, 2}));
  gtl::InlinedVector<TensorValue, 4> inputs{TensorValue(&a)};
  EXPECT_TRUE(GeOpRunGraphAsync(graph_def_path, inputs, node_def, "GeOp14_0").ok());
...
TEST_F(GeOpTest, GeOpAoeTuningTest) {
  ...
  for (int i = 0; i < graph_def.node_size(); i++) {
    ...
    if (node_def->name() == "GeOp1_1") {
      ...
      std::unique_ptr<DummyDevice> device1;
      std::unique_ptr<OpKernelContext> ctx1;
      for (int i = 0; i < train_graph_def.node_size(); i++) {
        ...
        if (node_def->name() == "GeOp2_0") {
          ...
          OpKernelContext::Params params;
          params.record_tensor_accesses = false;
          device1 = absl::make_unique<DummyDevice>(env, params.record_tensor_accesses);
          params.device = device1.get();
          Status status;
          std::unique_ptr<OpKernel> op(
              CreateOpKernel(DEVICE_CPU, params.device, cpu_allocator(), *node_def, TF_GRAPH_DEF_VERSION, &status));
          EXPECT_TRUE(status.ok());
          ...
          int forward_from = 0;
          params.forward_from_array = &forward_from;
          ctx1 = absl::make_unique<OpKernelContext>(&params);
          AsyncOpKernel::DoneCallback done = []() { LOG(INFO) << "DONE DoneCallback"; };
...
TEST_F(GeOpTest, RewriteInputShapeOptionTest) {
  std::string input_shape("a:-1,2;b:0");
  std::string target_input_shape("a:-1,2;b:");
  RewriteInputShapeOption(input_shape);
  EXPECT_TRUE(input_shape == target_input_shape);

  input_shape = "a:-1,2;b:0;";
  target_input_shape = "a:-1,2;b:;";
  RewriteInputShapeOption(input_shape);
  EXPECT_TRUE(input_shape == target_input_shape);

  input_shape = "a:-1,2;b:0,1,2";
  target_input_shape = "a:-1,2;b:0,1,2";
  RewriteInputShapeOption(input_shape);
  EXPECT_TRUE(input_shape == target_input_shape);
}
...
TEST_F(GeOpTest, GeOpNpuStringMaxSizeTest) {
  ...
  buff[SECUREC_MEM_MAX_LEN] = '\0';
  Tensor in(DT_STRING, TensorShape({
                           1,
                       }));
...
TEST_F(GeOpTest, GeOpCompileDynamicTest) {
  NodeDef node_def;
  std::string graph_def_path = "tf_adapter/tests/ut/kernels/pbtxt/geop_compile_dynamic.pbtxt";
  Tensor a(DT_INT32, TensorShape({1}));
  gtl::InlinedVector<TensorValue, 4> inputs{TensorValue(&a)};
  EXPECT_TRUE(GeOpRunGraphAsync(graph_def_path, inputs, node_def, "GeOp1_0").ok());
  Tensor b(DT_INT32, TensorShape({1, 2}));
  gtl::InlinedVector<TensorValue, 4> inputs2{TensorValue(&b)};
  EXPECT_TRUE(GeOpRunGraphAsync(graph_def_path, inputs2, node_def, "GeOp1_0").ok());
}

TEST_F(GeOpTest, GeOpDynamicTest) {
  NodeDef node_def;
  std::string graph_def_path = "tf_adapter/tests/ut/kernels/pbtxt/geop.pbtxt";
  Tensor a(DT_INT32, TensorShape({1}));
  gtl::InlinedVector<TensorValue, 4> inputs{TensorValue(&a)};
...
TEST_F(GeOpTest, GeOpShapeGeneralizationModeTest) {
  NodeDef node_def;
  std::string graph_def_path = "tf_adapter/tests/ut/kernels/pbtxt/geop_shape_generalization_mode.pbtxt";
  Tensor a(DT_INT32, TensorShape({2, 2}));
  gtl::InlinedVector<TensorValue, 4> inputs{TensorValue(&a)};
  EXPECT_TRUE(GeOpRunGraphAsync(graph_def_path, inputs, node_def, "GeOp1_0").ok());
  Tensor b(DT_INT32, TensorShape({1, 2}));
  gtl::InlinedVector<TensorValue, 4> inputs2{TensorValue(&b)};
  EXPECT_TRUE(GeOpRunGraphAsyncMultiStep(graph_def_path, {inputs2, inputs}, node_def, "GeOp1_0").ok());
}

TEST_F(GeOpTest, GeOpShapeGeneralizationModeIgnoreTest) {
  NodeDef node_def;
  std::string graph_def_path = "tf_adapter/tests/ut/kernels/pbtxt/geop_shape_generalization_mode_ignore.pbtxt";
  Tensor a(DT_INT32, TensorShape({2, 2}));
  gtl::InlinedVector<TensorValue, 4> inputs{TensorValue(&a)};
  EXPECT_TRUE(GeOpRunGraphAsync(graph_def_path, inputs, node_def, "GeOp1_0").ok());
  Tensor b(DT_INT32, TensorShape({1, 2}));
...
TEST_F(GeOpTest, GeOpTestEndOfSequence) {
  NodeDef node_def;
  std::string graph_def_path = "tf_adapter/tests/ut/kernels/pbtxt/geop.pbtxt";
  Tensor a(DT_INT32, TensorShape({1}));
  gtl::InlinedVector<TensorValue, 4> inputs{TensorValue(&a)};
  ge::RegRunGraphAsyncStub(
      [](uint32_t graphId, const std::vector<ge::Tensor> &inputs, ge::RunAsyncCallback callback) -> ge::Status {
        ...
        tensorflow::CallbackPack pack;
        pack.callback = callback;
        pack.ge_status = ge::END_OF_SEQUENCE;
        pack.outputs = outputs;
        tensorflow::CallbackExecutor::GetInstance().PushTask(pack);
        return ge::SUCCESS;
      });
  EXPECT_TRUE(GeOpRunGraphAsync(graph_def_path, inputs, node_def, "GeOp1_0").ok());
  ge::ClearRegRunGraphAsyncStub();
}
