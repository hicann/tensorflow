...
TEST(OCROpsTest, TestOCRDetectionPreHandleShapeInference) {
  ...
  NodeDef def;
  TF_CHECK_OK(
      NodeDefBuilder("dummy", &op_def).Attr("data_format", "NHWC").Input(FakeInputStub(DT_UINT8)).Finalize(&def));
  shape_inference::InferenceContext c(0, &def, op_def, {TShape({5, 5, 3})}, {}, {}, {});
  ASSERT_TRUE(reg->shape_inference_fn(&c).ok());
}
...
TEST(OCROpsTest, TestOCRDetectionPreHandleShapeInference1) {
  ...
  NodeDef def;
  TF_CHECK_OK(
      NodeDefBuilder("dummy", &op_def).Attr("data_format", "NCHW").Input(FakeInputStub(DT_UINT8)).Finalize(&def));
  shape_inference::InferenceContext c(0, &def, op_def, {TShape({3, 5, 5})}, {}, {}, {});
  ASSERT_TRUE(reg->shape_inference_fn(&c).ok());
}
...
TEST(OCROpsTest, TestOCRDetectionPreHandleShapeInferenceFail1) {
  ...
  TF_CHECK_OK(
      NodeDefBuilder("dummy", &op_def).Attr("data_format", "NHWC").Input(FakeInputStub(DT_UINT8)).Finalize(&def));
  shape_inference::InferenceContext c(0, &def, op_def, {TShape({5, 5, 4})}, {}, {}, {});
  ASSERT_TRUE(!reg->shape_inference_fn(&c).ok());
}
...
TEST(OCROpsTest, TestOCRDetectionPreHandleShapeInferenceFail2) {
  ...
  TF_CHECK_OK(
      NodeDefBuilder("dummy", &op_def).Attr("data_format", "NCHW").Input(FakeInputStub(DT_UINT8)).Finalize(&def));
  shape_inference::InferenceContext c(0, &def, op_def, {TShape({4, 5, 5})}, {}, {}, {});
  ASSERT_TRUE(!reg->shape_inference_fn(&c).ok());
}
...
TEST(OCROpsTest, TestOCRDetectionPreHandleShapeInferenceFail3) {
  ...
  TF_CHECK_OK(
      NodeDefBuilder("dummy", &op_def).Attr("data_format", "NCHW").Input(FakeInputStub(DT_UINT8)).Finalize(&def));
  shape_inference::InferenceContext c(0, &def, op_def, {TShape({3, 5, 5, 1})}, {}, {}, {});
  ASSERT_TRUE(!reg->shape_inference_fn(&c).ok());
}
...
TEST(OCROpsTest, TestOCRIdentifyPreHandleShapeInference) {
  ...
  TF_CHECK_OK(NodeDefBuilder("dummy", &op_def)
                  ...
                  .Input(FakeInputStub(DT_INT32))
                  .Finalize(&def));
  shape_inference::InferenceContext c(0, &def, op_def, {TShape({-1}), TShape({-1}), TShape({-1, 3})}, {}, {}, {});
  ASSERT_TRUE(reg->shape_inference_fn(&c).ok());
}
...
TEST(OCROpsTest, TestOCRIdentifyPreHandleShapeInferencefail1) {
  ...
  TF_CHECK_OK(NodeDefBuilder("dummy", &op_def)
                  ...
                  .Input(FakeInputStub(DT_INT32))
                  .Finalize(&def));
  shape_inference::InferenceContext c(0, &def, op_def, {TShape({-1, 1}), TShape({-1}), TShape({-1, 3})}, {}, {}, {});
  ASSERT_TRUE(!reg->shape_inference_fn(&c).ok());
}
...
TEST(OCROpsTest, TestOCRIdentifyPreHandleShapeInferencefail2) {
  ...
  TF_CHECK_OK(NodeDefBuilder("dummy", &op_def)
                  ...
                  .Input(FakeInputStub(DT_INT32))
                  .Finalize(&def));
  shape_inference::InferenceContext c(0, &def, op_def, {TShape({-1}), TShape({-1, -1}), TShape({-1, 3})}, {}, {}, {});
  ASSERT_TRUE(!reg->shape_inference_fn(&c).ok());
}
...
TEST(OCROpsTest, TestOCRIdentifyPreHandleShapeInferencefail3) {
  ...
  TF_CHECK_OK(NodeDefBuilder("dummy", &op_def)
                  ...
                  .Input(FakeInputStub(DT_INT32))
                  .Finalize(&def));
  shape_inference::InferenceContext c(0, &def, op_def, {TShape({-1}), TShape({-1}), TShape({-1, 4})}, {}, {}, {});
  ASSERT_TRUE(!reg->shape_inference_fn(&c).ok());
}
...
TEST(OCROpsTest, TestBatchDilatePolysShapeInference) {
  ...
  shape_inference::InferenceContext c(
      0, &def, op_def, {TShape({1}), TShape({1}), TShape({1}), TShape({1}), TShape({1}), TShape({1}), TShape({1})}, {},
      {}, {});
  ASSERT_TRUE(reg->shape_inference_fn(&c).ok());
}
...
TEST(OCROpsTest, TestOCRFindContoursShapeInference) {
  ...
  OpDef op_def = reg->op_def;
  NodeDef def;
  TF_CHECK_OK(NodeDefBuilder("dummy", &op_def).Attr("value_mode", 0).Input(FakeInputStub(DT_UINT8)).Finalize(&def));
  shape_inference::InferenceContext c(0, &def, op_def, {TShape({2})}, {}, {}, {});
  ASSERT_TRUE(reg->shape_inference_fn(&c).ok());
}
