...
TEST(ProdVirialSeAOpTest, TestProdVirialSeA) {
  std::vector<DataType> in_types_vec = {DT_FLOAT, DT_FLOAT, DT_FLOAT, DT_INT32, DT_INT32};
  ...
  ASSERT_FALSE(prod_virial_se_a.IsExpensive());
  delete device;
  delete node_def;
  delete op_def;
  delete context;
...
TEST(ProdVirialSeAOpTest, TestProdVirialSeAShapeInference) {
  ...
  shape_inference::InferenceContext c(
      0, &def, op_def,
      {TShape({1, 6782976}), TShape({1, 20348928}), TShape({1, 5087232}), TShape({1, 1695744}), TShape({4})}, {}, {},
      {});
