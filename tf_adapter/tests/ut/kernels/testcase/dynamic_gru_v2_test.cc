...
TEST(DynamicGRUV2OpTest, TestDynamicGRUV2) {
  std::vector<DataType> in_types_vec = {DT_FLOAT, DT_FLOAT, DT_FLOAT, DT_FLOAT, DT_FLOAT, DT_INT32, DT_FLOAT};
  DataTypeSlice input_types(in_types_vec);
  MemoryTypeSlice input_memory_types;
  std::vector<DataType> out_types_vec = {DT_FLOAT, DT_FLOAT, DT_FLOAT, DT_FLOAT, DT_FLOAT, DT_FLOAT};
  ...
  ASSERT_FALSE(dynamic_gru_v2.IsExpensive());
  delete device;
  delete node_def;
  delete op_def;
  delete context;
...
TEST(DynamicGRUV2OpTest, TestDynamicGRUV2ShapeInference) {
  ...
  shape_inference::InferenceContext c(0, &def, op_def,
                                      {TShape({1, 16, 16}), TShape({16, 48}), TShape({16, 48}), TShape({16, 16}),
                                       TShape({16, 16}), TShape({48}), TShape({16, 16})},
                                      {}, {}, {});
