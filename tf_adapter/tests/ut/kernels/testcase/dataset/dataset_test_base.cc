...
Status IsEqual2(const Tensor &tensor1, const Tensor &tensor2) {
  if (tensor1.dtype() != tensor2.dtype()) {
    return tensorflow::errors::Internal("Tensor1 and tensor2 is different in dtypes: ", DataTypeString(tensor1.dtype()),
                                        " and. ", DataTypeString(tensor2.dtype()));
  }
  if (!tensor1.IsSameSize(tensor2)) {
    return tensorflow::errors::Internal("Tensor1 and tensor2 is different in shape: ", tensor1.shape().DebugString(),
                                        " and. ", tensor2.shape().DebugString());
  }
...
Status DatasetOpsTestBase::ExpectEqual(std::vector<Tensor> tensors1, std::vector<Tensor> tensors2, bool order) {
  if (tensors1.size() != tensors2.size()) {
    return Status(tensorflow::errors::Internal("The two tensor vectors have different size (", tensors1.size(),
                                               " and. ", tensors2.size(), ")"));
  }

  if (tensors1.empty()) return Status::OK();
  if (tensors1[0].dtype() != tensors2[0].dtype()) {
    return Status(tensorflow::errors::Internal("The two tensor vectors have different dtypes (", tensors1[0].dtype(),
                                               " and . ", tensors2[0].dtype(), ")"));
  }
...
#define case01(DT)                                                                  \
  case DT:                                                                          \
    std::sort(tensors1.begin(), tensors1.end(), compare<EnumToDataType<DT>::Type>); \
    std::sort(tensors2.begin(), tensors2.end(), compare<EnumToDataType<DT>::Type>); \
    break;
      case01(DT_FLOAT);
      case01(DT_DOUBLE);
      case01(DT_INT32);
...
#undef case
  ...
  for (int i = 0; i < tensors1.size(); ++i) {
    TF_RETURN_IF_ERROR(DatasetOpsTestBase::ExpectEqual(tensors1[i], tensors2[i]));
  }
  return Status::OK();
}

Status DatasetOpsTestBase::CreateTensorSliceDatasetKernel(StringPiece node_name, const DataTypeVector &dtypes,
                                                          const std::vector<PartialTensorShape> &shape,
  ...
  NodeDef node_def =
      test::function::NDef(node_name, "TensorSliceDataset", com, {{"Toutput_types", dtypes}, {"output_shapes", shape}});
  TF_RETURN_IF_ERROR(CreateOpKernel(node_def, dataset_kernel));
  return Status::OK();
}

Status DatasetOpsTestBase::CreateTensorSliceDataset(StringPiece node_name, std::vector<Tensor> *const com,
                                                    DatasetBase **slice_data) {
...
Status DatasetOpsTestBase::MakeRangeDataset(const Tensor &rt, const Tensor &sp, const Tensor &st,
  ...
  TF_RETURN_IF_ERROR(
      RunFunction(test::function::MakeRangeDataset(),
                  {{RangeDatasetOp::kOutputTypes, output_types}, {RangeDatasetOp::kOutputShapes, output_shapes}},
...
Status DatasetOpsTestBase::MakeRangeDataset(const RangeDatasetParams &params, Tensor *range_dataset) {
  ...
  TF_RETURN_IF_ERROR(RunFunction(
      ...
      {{RangeDatasetOp::kOutputTypes, params.output_dtypes}, {RangeDatasetOp::kOutputShapes, params.output_shapes}},
      {params.start, params.stop, params.step}, opts, {range_dataset}));
  return Status::OK();
}

Status DatasetOpsTestBase::MakeTakeDataset(const Tensor &input, int64 count, const DataTypeVector &types,
                                           const std::vector<PartialTensorShape> &shapes, Tensor *take_dataset) {
  GraphConstructorOptions opts;
  opts.allow_internal_ops = true;
  opts.expect_device_spec = false;

  Tensor count_tensor = CreateTensor<int64>(TensorShape({}), {count});
  TF_RETURN_IF_ERROR(RunFunction(test::function::MakeTakeDataset(),
                                 {{TakeDatasetOp::kOutputTypes, types}, {TakeDatasetOp::kOutputShapes, shapes}},
                                 {input, count_tensor}, opts, {take_dataset}));
  return Status::OK();
}

Status DatasetOpsTestBase::CreateOpKernel(const NodeDef &node_def, std::unique_ptr<OpKernel> *op_kernels) {
  OpKernel *kernels;
  TF_RETURN_IF_ERROR(tensorflow::CreateOpKernel(device_type_, device_.get(), allocator_, flr_, node_def,
                                                TF_GRAPH_DEF_VERSION, &kernels));
  op_kernels->reset(kernels);
  return Status::OK();
}

Status DatasetOpsTestBase::CreateDatasetContext(OpKernel *const data_set_kernel,
                                                gtl::InlinedVector<TensorValue, 4> *const nums,
                                                std::unique_ptr<OpKernelContext> *context) {
...
Status DatasetOpsTestBase::CreateDataset(OpKernel *data_set_kernel, OpKernelContext *context_data,
                                         DatasetBase **const dataset_test_base) {
...
Status DatasetOpsTestBase::RestoreIterator(IteratorContext *context, IteratorStateReader *readers,
...
Status DatasetOpsTestBase::CreateIteratorContext(OpKernelContext *const context,
                                                 std::unique_ptr<IteratorContext> *iterator) {
...
Status DatasetOpsTestBase::GetDatasetFromContext(OpKernelContext *context, int index,
                                                 DatasetBase **const dataset_base) {
...
Status DatasetOpsTestBase::InitFunctionLibraryRuntime(const std::vector<FunctionDef> &flib, int num) {
  ...
  pflr_ = absl::make_unique<ProcessFunctionLibraryRuntime>(device_mgr_.get(), Env::Default(), TF_GRAPH_DEF_VERSION,
                                                           lib_def_.get(), options, thread_pool_.get(), nullptr);
...
Status DatasetOpsTestBase::RunFunction(const FunctionDef &fdef1, test::function::Attrs attr,
                                       const std::vector<Tensor> &arg, const GraphConstructorOptions &gps,
                                       std::vector<Tensor *> rets) {
  std::unique_ptr<Executor> exec;
  InstantiationResult ret;
  auto GetOpSig = [](const string &op, const OpDef **sig) { return OpRegistry::Global()->LookUpOpDef(op, sig); };
  ...
  pas.delete_kernel = [](OpKernel *kernel) { DeleteNonCachedKernel(kernel); };
...
Status DatasetOpsTestBase::CreateOpKernelContext(OpKernel *kernels, gtl::InlinedVector<TensorValue, 4> *inputs,
                                                 std::unique_ptr<OpKernelContext> *ctx) {
  params_ = absl::make_unique<OpKernelContext::Params>();
  OpKernelContext::Params *pas = params_.get();
  cancellation_manager_ = absl::make_unique<CancellationManager>();
  pas->cancellation_manager = cancellation_manager_.get();
  pas->device = device_.get();
  ...
  pas->step_container = step_container_.get();
  ...
  *ctx = absl::make_unique<OpKernelContext>(pas);
  return Status::OK();
}
...
Status DatasetOpsTestBase::CheckOpKernelInput(const OpKernel &kernel,
...
Status DatasetOpsTestBase::AddDatasetInput(gtl::InlinedVector<TensorValue, 4> *inputs, DataTypeVector input_types,
...
Status DatasetOpsTestBase::CheckIteratorOutputDtypes(const DataTypeVector &dtypes) {
  TF_EXPECT_OK(VerifyTypesMatch(iterator_->output_dtypes(), dtypes));
  return Status::OK();
}
