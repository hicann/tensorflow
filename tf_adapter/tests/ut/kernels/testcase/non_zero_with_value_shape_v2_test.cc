...
TEST(NonZeroWithValueShapeV2OpsTest, TestNonZeroWithValueShapeV2) {
  std::vector<DataType> in_types_vec = {DT_INT32, DT_INT32, DT_INT32};
  DataTypeSlice input_types(in_types_vec);
  MemoryTypeSlice input_memory_types;
  std::vector<DataType> out_types_vec = {DT_INT32, DT_INT32};
  ...
  NonZeroWithValueShapeV2Op cache(context);
...
TEST(NonZeroWithValueShapeV2OpsTest, TestNonZeroWithValueShapeV2Infer) {
  const OpRegistrationData *reg;
  TF_CHECK_OK(OpRegistry::Global()->LookUp("NonZeroWithValueShapeV2", &reg));
