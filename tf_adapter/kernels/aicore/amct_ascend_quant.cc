...
int AscendQuantInternelCpu(struct QuantInputParam<T> input_param) {
  int bound = static_cast<int>(pow(BASE, input_param.quant_bits - 1));
...
REGISTER_KERNEL_BUILDER(Name("AscendQuant").Device(tensorflow::DEVICE_CPU).TypeConstraint<float>("T"),
                        AscendQuantOp<float>);
