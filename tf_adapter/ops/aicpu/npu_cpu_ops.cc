...
REGISTER_OP("DeformableOffsets")
    ...
    .SetShapeFn([](shape_inference::InferenceContext *c) {
      ...
      if (ksizes.size() != 2) {
        return errors::InvalidArgument("ksize attribute should contain 2 values, but got: ", ksizes.size());
      }
...
REGISTER_OP("RandomChoiceWithMask")
    ...
    .SetShapeFn([](shape_inference::InferenceContext *c) {
      ...
      if (count > 0) {
        ...
        return errors::InvalidArgument("input count must greater or equal to 0 but instead is ", count);
      }
      return Status::OK();
    });
...
REGISTER_OP("OCRIdentifyPreHandle")
    ...
    .SetShapeFn([](shape_inference::InferenceContext *c) {
      ...
      if (size.size() != 2) {
        return errors::InvalidArgument("size attribute should contain 2 values, but got: ", size.size());
      }
      ...
      if (c->Rank(c->input(0)) != kImgShapeRank) {
        return errors::InvalidArgument("Invalid images shape: must be 1, bug got: ", c->Rank(c->input(0)));
      }
      const int32 kImgOffsetShapeRank = 1;
      if (c->Rank(c->input(1)) != kImgOffsetShapeRank) {
        return errors::InvalidArgument("Invalid images offset shape: must be 1, bug got: ", c->Rank(c->input(1)));
      }
      const int32 kImgSizeShapeRank = 2;
      if (c->Rank(c->input(2)) != kImgSizeShapeRank) {
        return errors::InvalidArgument("Invalid images size shape: must be 2, bug got: ", c->Rank(c->input(2)));
      }
...
REGISTER_OP("Dequeue")
    ...
    .Attr(
        "output_type: {float16, float32, float64, int8, uint8,"
...
REGISTER_OP("NonZeroWithValueShapeV2")
    ...
    .Input("index: int32")
    .Input("count: int32")
    .Output("value_output: T")
    .Output("index_output: int32")
    .Attr("T: {double, float, float16, int8, uint8, int16, uint16, int32, uint32, int64, uint64, bool}")
...
Status DecodeImageV3ShapeFn(InferenceContext *c) {
  ...
  if (channels == 0) {
    ...
    if (channels < 0) {
      return errors::InvalidArgument("channels must be non-negative, got ", channels);
    }
  ...
  if (expand_animations) {
    ...
    c->set_output(0, c->MakeShape({InferenceContext::kUnknownDim, InferenceContext::kUnknownDim, channels_dim}));
    return Status::OK();
  }
}
