...
class StringUtils {
 public:
  static std::vector<std::string, std::allocator<std::string>> Split(const std::string &str, const char delim);
};
...
class NpuAttrs {
 public:
  ...
  static std::map<std::string, std::string> GetGraphOptions(const OpKernelConstruction *ctx);
  ...
  static void SetForbiddenClosePassOn(std::map<std::string, std::string> &option);
