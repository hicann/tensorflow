...
bool NpuOpsIdentifier::GetOppPluginVendors(const std::string &vendors_config, std::vector<std::string> &vendors) {
  ...
  std::vector<std::string> v_parts = tensorflow::StringUtils::Split(content, '=');
  if (v_parts.size() != kVendorConfigPartsCount) {
    ADP_LOG(ERROR) << "Format of file content is invalid!";
    return false;
  }
  vendors = tensorflow::StringUtils::Split(v_parts[1], ',');
  if (vendors.empty()) {
    ADP_LOG(ERROR) << "Format of file content is invalid!";
    return false;
  }
...
void NpuOpsIdentifier::GetCustomOpPathFromCustomOppPath(std::vector<std::string> &custom_ops_json_path_vec) {
  ADP_LOG(INFO) << "Start to get custom ops json path from custom opp path schedule.";
  ...
  ADP_LOG(INFO) << "value of custom opp path is " << custom_opp_path << ".";
  std::vector<std::string> custom_paths = tensorflow::StringUtils::Split(custom_opp_path, ':');
