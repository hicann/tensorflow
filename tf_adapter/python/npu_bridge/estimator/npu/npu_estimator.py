...
class NPUEstimator(estimator_lib.Estimator):
    ...
    def __check_profiling_options(self, profiling_options=()):
        ...
        Return:
            Valid options
        Raise:
            If profiling_options is null or option is not `training_trace` or `task_time`, `op_trace`'.
        """

        error_mag = 'profiling options must be in `training_trace`, `task_time` or `op_trace`'

        if len(profiling_options) == 0:
            raise ValueError(error_mag)

        profiling_types = ["training_trace", "task_time", "op_trace"]
    ...
    def __load_memory_config(self, config, custom_op):
        ...
        if config._memory_config is not None:
            ...
            if config._memory_config._variable_use_1g_huge_page is not None:
                custom_op.parameter_map[
                    "variable_use_1g_huge_page"].i = config._memory_config._variable_use_1g_huge_page
    ...
    def __load_dynamic_input_config(self, config, custom_op):
        ...
        if (config._dynamic_input_config is not None and
                ...
                config._dynamic_input_config._dynamic_node_type is not None and
                config._dynamic_input_config._compile_hybrid_mode is not None):
            ...
            custom_op.parameter_map["compile_hybrid_mode"].i = config._dynamic_input_config._compile_hybrid_mode
    ...
    def __load_export_compile_stat(self, config, custom_op):
        """Load export_compile_stat config, and add to custom_optimizers
        Args:
            config: NPURunConfig.
            custom_op: Customer optimizers.
        """
    ...
    def __oo_constant_folding(self, config, custom_op):
        """Load oo_constant_folding config, and add to custom_optimizers
        Args:
            config: NPURunConfig.
            custom_op: Customer optimizers.
        """
        if config._oo_constant_folding is not None:
            custom_op.parameter_map["oo_constant_folding"].b = config._oo_constant_folding

    def __load_input_batch_cpy(self, config, custom_op):
        """Load input_batch_cpy config, and add to custom_optimizers
        Args:
            config: NPURunConfig.
            custom_op: Customer optimizers.
        """
        if config._input_batch_cpy is not None:
            custom_op.parameter_map["input_batch_cpy"].b = config._input_batch_cpy

    def __load_oo_level(self, config, custom_op):
        """Load oo_level config, and add to custom_optimizers
        Args:
            config: NPURunConfig.
            custom_op: Customer optimizers.
        """
        if config._oo_level is not None:
            custom_op.parameter_map["oo_level"].s = tf.compat.as_bytes(config._oo_level)

    def __load_optimization_switch(self, config, custom_op):
        """Load optimization_switch config, and add to custom_optimizers
        Args:
            config: NPURunConfig.
            custom_op: Customer optimizers.
        """
        if config._optimization_switch is not None:
            custom_op.parameter_map["optimization_switch"].s = tf.compat.as_bytes(config._optimization_switch)
    ...
    def __load_graph_optimizers(self, config):
        ...
        if config._compile_dynamic_mode is not None:
            custom_op.parameter_map["compile_dynamic_mode"].b = config._compile_dynamic_mode
        if config._graph_max_parallel_model_num is not None:
            custom_op.parameter_map["graph_max_parallel_model_num"].i = config._graph_max_parallel_model_num
        custom_op.parameter_map["jit_compile"].s = tf.compat.as_bytes(config._jit_compile)
        custom_op.parameter_map["input_fusion_size"].i = config._input_fusion_size
        ...
        if config._shape_generalization_mode is not None:
            custom_op.parameter_map["shape_generalization_mode"].s = tf.compat.as_bytes(
