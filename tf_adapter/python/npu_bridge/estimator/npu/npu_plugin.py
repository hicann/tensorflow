...
def npu_resource_init(graph_run_mode=1,
    ...
    if oo_constant_folding is not None:
        util.check_bool_type(oo_constant_folding, "oo_constant_folding")
        init["ge.oo.constantFolding"] = "true" if oo_constant_folding is True else "false"
    if input_batch_cpy is not None:
        util.check_bool_type(input_batch_cpy, "input_batch_cpy")
        init["ge.inputBatchCpy"] = "true" if input_batch_cpy is True else "false"
    if oo_level is not None:
        init["ge.oo.level"] = str(oo_level)
    if optimization_switch is not None:
        init["ge.optimization_switch"] = str(optimization_switch)
    ...
    for var in remote_var_list:
        for line in var:
            ...
            if server_id == var_server_id:
                var_addr_list.append((line[1], line[2]))
...
def rdma_remote_init(remote_var_list, mem_size):
    ...
    for var in remote_var_list:
        ...
        var_addr_list.append((server_var[1], server_var[2]))
...
def malloc_shared_memory(var_name, shape, data_type):
    ...
    """
    res = tf_adapter.MallocSharedMem(var_name, tf_adapter.int64_vec(shape), data_type)
    if res[0] != 0:
