...
- **Python3.8~Python3.11**
  ...
  Ascend Adapter可以使用Python3.8, Python3.9, Python3.10, Python3.11版本进行编译。

- **TensorFlow 2.13.0**

  Ascend Adapter 与 Tensorflow 有严格的匹配关系，从源码构建前，您需要确保已经正确安装了[Tensorflow v2.13.0 版本](https://www.tensorflow.org/install) 

- **GCC >= 9.3.1**

  Ascend Adapter 需要使用9.3.1及更高版本的gcc编译, 建议使用11.4.0版本进行编译。

- **CMake >= 3.20.0**

  Ascend Adapter 需要使用3.20.0及更高版本的cmake编译， 建议使用3.28.0版本进行编译。
...
```
./build/dist/python/dist/npu_device-2.13.0-py3-none-manylinux2014_<arch>.whl
```
...
```
pip3 install ./build/dist/python/dist/npu_device-2.13.0-py3-none-manylinux2014_<arch>.whl --upgrade
```
> [!NOTE]说明
