...
std::string NpuDevice::CreateDevice(const char *name, int device_index,
  ...
  *device = new (std::nothrow) NpuDevice();
  ...
  (*device)->device_options = global_options;
  for (const auto &option : session_options) {
    (*device)->device_options.emplace(option);
  }
...
TFE_TensorHandle *NpuDevice::NewDeviceTensorHandle(TFE_Context *context, Format fmt,
  ...
  auto buf = new TF_ManagedBuffer(npu_managed_buffer, sizeof(NpuManagedBuffer), &npu::NpuManagedBufferDeallocator,
                                  nullptr, false);
...
tensorflow::Status NpuDevice::InferShape(const TFE_Context *const context,
  ...
  for (int i = 0; i < num_inputs; i++) {
    ...
    if (npu::IsNpuTensorHandle(input)) {
      std::unique_ptr<TF_Status, decltype(&TF_DeleteStatus)> s(TF_NewStatus(), TF_DeleteStatus);
      if (s == nullptr) {
        continue;
      }
      input = CopyTensorD2H(context, input, *s);
      if (TF_GetCode(s.get()) != TF_OK) {
        continue;
      }
    ...
    for (size_t i = 0; i < ge_outputs.size(); i++) {
      ...
      if (reinterpret_cast<uintptr_t>(ge_tensor.GetData()) % kTensorAlignBytes == 0) {
        ...
        tensorflow::Allocator *allocator =
          NpuHostFixedAllocator<uint8_t[], std::function<void(uint8_t *)>>::Create(ge_tensor.ResetData());
...
void NpuDevice::TransTfInputs2GeInputs(int num_inputs, TFE_TensorHandle **inputs, TF_Status &status,
  ...
  for (int i = 0; i < num_inputs; i++) {
    ...
    ge::DataType ge_type;
    (void)GeApiWrapper_GetGeDataTypeByTFType(tensor->dtype(), ge_type);
    NPU_CTX_REQUIRES(
    ...
    input.SetData(static_cast<uint8_t *>(tensor->data()), tensor->TotalBytes(), [](uint8_t *) {});
...
uint64_t NpuDevice::AddGeGraphInner(TFE_Context *context, uint64_t graph_id, const std::string &name,
  ...
  size_t num_inputs = GeApiWrapper_GetComputeGraphInputSize(ge_graph);
  size_t num_outputs = GeApiWrapper_GetComputeGraphOutputSize(ge_graph);
...
tensorflow::Status NpuDevice::TransTfGraph2GeGraph(TFE_Context *context, const std::string &name,
  ...
  ge::ComputeGraphPtr ge_compute_graph = GeApiWrapper_MakeComputeGraphPtr(name.c_str());
...
tensorflow::Status LoadOpsFromJson(const std::string json_file_path, std::unordered_set<std::string> &ops) {
  NPU_REQUIRES_OK(tensorflow::Env::Default()->FileExists(json_file_path));
  std::ifstream fs(json_file_path, std::ifstream::in);
  NPU_REQUIRES(fs.is_open(), tensorflow::errors::Internal("Failed open config file ", json_file_path));
  nlohmann::json root;
  try {
    fs >> root;
  } catch (nlohmann::json::exception &e) {
    fs.close();
    return tensorflow::errors::Internal("Parse json from ", json_file_path, " failed ", e.what());
  }
...
tensorflow::Status NpuDevice::LoadCustomSupportedOps() {
  const char *const custom_opp_path_env = aclGetCustomOpLibPath();
  if (custom_opp_path_env == nullptr) {
    DLOG() << "env ASCEND_CUSTOM_OPP_PATH is not defined.";
    return tensorflow::errors::Internal("ASCEND_CUSTOM_OPP_PATH not defined");
  }
  const std::string custom_opp_path = custom_opp_path_env;
  if (custom_opp_path.empty()) {
    DLOG() << "env ASCEND_CUSTOM_OPP_PATH is defined but it's empty.";
    return tensorflow::errors::Internal("ASCEND_CUSTOM_OPP_PATH empty");
  }
  DLOG() << "value of custom opp path is " << custom_opp_path << ".";
  std::vector<std::string> custom_paths = npu::StringUtils::Split(custom_opp_path, ':');
  for (const auto &custom_path : custom_paths) {
    std::string custom_ops_json_path = custom_path + "/framework/tensorflow/npu_supported_ops.json";
    if (!custom_path.empty() && tensorflow::Env::Default()->FileExists(custom_ops_json_path).ok()) {
      DLOG() << "custom_path '" << custom_ops_json_path << "' is valid.";
      auto res = LoadOpsFromJson(custom_ops_json_path, npu_supported_custom_ops_);
      if (res.ok()) {
...
tensorflow::Status NpuDevice::LoadSupportedOps() {
  ...
  auto res = LoadOpsFromJson(supported_ops_json, npu_supported_ops_);
  if (res.ok()) {
